...
void main() {
    float flicker = 0.85 + 0.15 * noise(v_prog * 100.0 + u_time);

    vec3 deep  = vec3(0.17, 0.05, 0.05);
    vec3 hot   = vec3(1.0, 0.25, 0.0);
    vec3 white = vec3(1.0, 0.95, 0.78);

    vec3 col = mix(deep, hot, smoothstep(0.0, 0.65, v_prog));
    col = mix(col, white, smoothstep(0.65, 1.0, v_prog));
...
class RosslerCanvas(app.Canvas):
    def __init__(self):
        super().__init__(size=(1200, 800),
                         title="Rössler Attractor — Fire Glow (Zoom Enabled)",
                         keys='interactive')
    ...
    def on_timer(self, event):
        ...
        self.build = min(1.0, self.build + 0.0012)
